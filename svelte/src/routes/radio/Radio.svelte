<script lang="js">
  ...
  onMount(async () => {
    ...
    // Start logging longitude and latitude at a certain interval
    startLoggingCoordinates(5000); // Log every 5 seconds
  });
  ...
  function startLoggingCoordinates(interval) {
    LongLatIntervalID = setInterval(async () => {
      try {
        const longitude = await getDatarefValue(LONGITUDE_ID);
        const latitude = await getDatarefValue(LATITUDE_ID);
